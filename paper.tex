...
\begin{itemize}
  \item None of the tools used in practice support foreign keys. These tools will not work when a foreign key is present on a table under change. This forces software engineers to choose between not using any of these tools, or deliberately avoid defining foreign key constraints in their SQL databases in order to be able to use these tools.
...
\begin{itemize}
  ...
  \item{Clients connected to the database must be able to access the contents of the database according to an active version of the database schema of their choosing. Their ``view'' should only contain tables, columns, foreign keys, etc which are present in their chosen version of the database schema.}
...
\begin{figure}%
    \centering
    \subfloat[Original database schema]{{\includegraphics[width=2.75in]{pre-migration-state.png} }}%
    \qquad
    \subfloat[Database schema in mixed-state]{{\includegraphics[width=2.75in]{in-migration-state.png} }}%
    \qquad
    \subfloat[New database schema]{{\includegraphics[width=2.75in]{post-migration-state.png} }}%
